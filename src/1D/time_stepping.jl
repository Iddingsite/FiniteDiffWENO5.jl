...
"""
    WENO_scheme!(u::T, v, weno::WENOScheme, Δt, Δx) where T <: AbstractVector{<:Real}
...
"""
function WENO_scheme!(u::T, v, weno::WENOScheme, Δt, Δx) where T <: AbstractVector{<:Real}
    ...
    @inbounds for i = axes(ut, 1)
        ut[i] = 0.75 * u[i] + 0.25 * ut[i] - 0.25 * Δt * du[i]
    end
    ...
    @inbounds for i = axes(u, 1)
        u[i] = 1.0/3.0 * u[i] + 2.0/3.0 * ut[i] - (2.0/3.0) * Δt * du[i]
    end
